...
class QuantAdd(torch.nn.Module):
    ...
    def __init__(self, quantization, num_bits=8):
        super().__init__()

        if quantization:
            self._input0_quantizer = quant_nn.TensorQuantizer(QuantDescriptor(num_bits=num_bits, calib_method="histogram"))
            self._input1_quantizer = quant_nn.TensorQuantizer(QuantDescriptor(num_bits=num_bits, calib_method="histogram"))
...
class QuantConcat(torch.nn.Module):
    ...
    def __init__(self, quantization=True, num_bits=8):
        super().__init__()
        self.quantization = quantization
    ...
    def add_input_quantizer(self):
        if self.quantization:
            quantizer = quant_nn.TensorQuantizer(QuantDescriptor(num_bits=self.num_bits, calib_method="histogram"))
...
class QuantChunk(torch.nn.Module):
    def __init__(self, c, num_bits=8):
        super().__init__()
        self._input0_quantizer = quant_nn.TensorQuantizer(QuantDescriptor(num_bits=num_bits, calib_method="histogram"))
        self._input0_quantizer._calibrator._torch_hist = True
        self.c = c
...
class QuantUpsample(torch.nn.Module):
    def __init__(self, size=None, scale_factor=None, mode='nearest', align_corners=None, quantization=True, num_bits=8):
        super().__init__()
        self.size = size
        ...
        if quantization:
            self._input_quantizer = quant_nn.TensorQuantizer(QuantDescriptor(num_bits=num_bits, calib_method="histogram"))
...
def set_quantizer_bits(model: torch.nn.Module, layer_patterns: List[str], num_bits: int):
    """Set num_bits for all TensorQuantizers inside modules whose
    fully-qualified name matches any regex in layer_patterns.

    Must be called after replace_to_quantization_module() and before
    calibrate_model() so the calibrator collects stats at the correct bit width.

    Args:
        model:          Quantized model (after replace_to_quantization_module).
        layer_patterns: List of regex strings, e.g. [r"model\\.6\\..*"].
        num_bits:       Target bit width (4 or 8).
    """
    changed = 0
        ...
        if any(re.match(p, name) for p in layer_patterns):
            for _, submodule in module.named_modules():
                if isinstance(submodule, quant_nn.TensorQuantizer):
                    ...
                    if submodule._calibrator is not None and hasattr(submodule._calibrator, '_num_bits'):
                        submodule._calibrator._num_bits = num_bits
                    changed += 1
    print(f"[Mixed Precision] Set {num_bits}-bit on {changed} quantizer(s) matching {layer_patterns}")


def apply_mixed_precision(model: torch.nn.Module, mixed_precision_config: Dict[int, List[str]]):
    """Apply mixed precision by setting different bit widths per layer group.

    Args:
        model: Quantized model (after replace_to_quantization_module).
        mixed_precision_config: Mapping of {num_bits: [regex_patterns]}.
            Example:
            {
                4: [r"model\\.6\\..*", r"model\\.7\\..*"],
                8: []   # empty → leave the rest untouched (already INT8)
            }
    """
    for num_bits, patterns in mixed_precision_config.items():
        if patterns:
            set_quantizer_bits(model, patterns, num_bits)


def print_quantizer_bits(model: torch.nn.Module):
    """Print the bit width of every TensorQuantizer in the model.
    Use during development to verify mixed precision assignments.
    """
    bit_counts: Dict[int, int] = {}
    for name, module in model.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer):
            bits = module._num_bits
...
def _cast_quantizer_scales_to_float32(model: torch.nn.Module):
    """Ensure all TensorQuantizer scale tensors are float32.

    pytorch_quantization stores calibrated amax values as Python floats,
    which TorchScript traces as float64 (Double). aten::fake_quantize_per_tensor_affine
    requires the scale to be float32 (Float), causing a crash at export time.
    This function fixes any quantizer whose _amax or _scale is still float64
    or a raw Python float.
    """
    fixed = 0
    for name, module in model.named_modules():
        ...
        # Use getattr: some TensorQuantizer instances (e.g. inside NMS wrappers)
        # never have _amax set at all — direct access raises AttributeError.
        amax = getattr(module, '_amax', None)
        if amax is not None:
            if isinstance(amax, torch.Tensor):
                if amax.dtype != torch.float32:
                    module._amax = amax.to(dtype=torch.float32)
                    fixed += 1
            else:
                # Plain Python float — convert to a float32 scalar tensor
                module._amax = torch.tensor(float(amax), dtype=torch.float32)
                fixed += 1

        if hasattr(module, '_scale') and module._scale is not None:
            if isinstance(module._scale, torch.Tensor):
                if module._scale.dtype != torch.float32:
                    module._scale = module._scale.to(dtype=torch.float32)
                    fixed += 1
            else:
                module._scale = torch.tensor(float(module._scale), dtype=torch.float32)
                fixed += 1

    if fixed:
        print(f"[export_onnx] Cast {fixed} quantizer scale tensor(s) to float32")


def _save_and_promote_int4_quantizers(model: torch.nn.Module) -> dict:
    """Temporarily promote INT4 quantizers to INT8 for ONNX export.

    ONNX opset 13 fake_quantize_per_tensor_affine only accepts standard INT8
    ranges (0,127), (0,255), (-128,127).  INT4 range (-8,7) causes a
    SymbolicValueError at export time.  This function saves the original bit
    width for every INT4 quantizer and sets it to 8 so the export succeeds.
    Call _restore_quantizer_bits() in the finally block to undo the change.

    Returns:
        dict mapping TensorQuantizer instance → original num_bits (int).
    """
    saved = {}
    for _, module in model.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer) and getattr(module, '_num_bits', 8) == 4:
            saved[module] = module._num_bits
            module._num_bits = 8
    if saved:
        print(f"[export_onnx] Promoted {len(saved)} INT4 quantizer(s) to INT8 for ONNX export")
    return saved
...
def export_onnx(model, input, file, *args, **kwargs):
    # Safety reset: if a previous export crashed before the flag was cleared,
    # use_fb_fake_quant stays True globally and poisons subsequent training passes.
    quant_nn.TensorQuantizer.use_fb_fake_quant = False

    # pytorch_quantization does not support ONNX export in FP16.
    # Temporarily cast the model and input to FP32, then restore.
    model_was_half = next(model.parameters()).dtype == torch.float16
    if model_was_half:
        model.float()

    # Guard: ensure all quantizer scales are float32 before tracing.
    # Covers models loaded from checkpoints saved before the calibration fix.
    _cast_quantizer_scales_to_float32(model)

    # ONNX opset 13 rejects INT4 range (-8, 7). Promote INT4 → INT8 for the
    # duration of the export, then restore in the finally block.
    ...
    try:
        ...
        # Always restore the flag, bit widths, and model dtype even if export crashes.
        quant_nn.TensorQuantizer.use_fb_fake_quant = False
            ...
            if isinstance(module, quant_nn.TensorQuantizer):
                if module._calibrator is not None:
                    ...
                    if module._amax is not None:
                        # Cast to float32: TorchScript traces Python float as
                        # float64 (Double), but fake_quantize kernels require
                        # the scale to be float32 (Float). Casting here fixes
                        # the "Double scale" error during ONNX export.
                        module._amax = module._amax.to(device=device, dtype=torch.float32)
