...
def export_formats():
    ...
    x = [
        ...
        ["ONNX TensorRT", "onnx_trt", "_trt.onnx", True, True, ["batch", "dynamic", "half", "opset", "simplify", "nms"]],
    ]
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if sum(flags) != 1:
            raise ValueError(f"Invalid export format='{fmt}'. Valid formats are {fmts}")
        (jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, paddle, mnn, ncnn, imx, rknn, onnx_trt) = (
